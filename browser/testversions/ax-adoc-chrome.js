...
var SERVER_URL = 'https://axc2.axiros.com/asciidoctor'
...
function make_theme_switchers() {
    ...
    docbody().insertBefore(t, docbody().firstChild)
    return
...
function wait_rendered() {
    ...
    //     return setTimeout(wait_rendered, 100)
    // }
    D.TH.index = {adoc: ['plain', 'fedora'], code: ['github', 'arta']}
    console.log(4)
    if (ext_has_css_set()) D.TH.has_css_set = true
    if (D.TH.index == 'offline') return
    D.axattrs.themes && setTimeout(make_theme_switchers, 100)
    console.log(5)
    // sets stored themes:
...
if (D.axattrs) {
    // rerendering in live preview mode (after polling source changes):
    if (D.TH.has_css_set) {
        // then it would set ITS CSS again :-/, so:
        set_theme(false, 'adoc')
        set_theme(false, 'code')
    }
}
...
if (!D.axattrs) {
    ...
    // when we are not allowed to access local store we need a fallback:
    D.session_store = {
        setItem: function(k, v) {
            D.session_store[k] = v
        },
        getItem: function(k) {
            return D.session_store[k]
        },
    }
